...
public class Gaussian {
    ...
    private double[][] srcImageData, outImageData;
    /**
     * 高斯模糊的构造函数
     ...
     * @param outImage 结果图像
     */
    public Gaussian(double sigam, int normalizationMode, int dimension,double[][] srcImage,double[][] outImage){
        this.sigam=sigam;
        this.normalizationMode =normalizationMode;
        this.srcImageData =srcImage;
        this.outImageData =outImage;
    ...
    public void gaussianPicture(){
        ...
        try {
            switch (gaussianKenel.length){
                ...
                case 1:
                    ImageBaseOp.Convolution(srcImageData, outImageData,gaussianKenel[0],getModulusOfNormalization());
                    break;
                //使用二维高斯模板一次卷积 O(n*n*M*N)
                default:
                    ImageBaseOp.Convolution(srcImageData, outImageData,gaussianKenel,getModulusOfNormalization());
                    break;
            }
        }catch (Exception e){
            e.printStackTrace();
